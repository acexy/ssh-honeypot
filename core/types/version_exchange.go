...
type VersionExchangeComponent interface {
	...
	// RegisterServerVersionStrategy 注册响应服务端版本处理的所有策略
	RegisterServerVersionStrategy() map[string]ShowServerVersionStrategy
	...
	// RegisterClientVersionStrategy 注册检查客户端版本处理策略
	RegisterClientVersionStrategy() map[string]HandleClientVersionStrategy
...
type ShowServerVersionStrategy interface {
	...
	ShowVersion(request *SSHRequest) (disconnect bool, delayResponseSec int, serverVersion string)
}

// HandleClientVersionStrategy 检查客户端版本处理策略
type HandleClientVersionStrategy interface {
	HandleVersion(request *SSHRequest, clientVersion string) bool
}
